...
class DeflaterEngine implements DeflaterConstants {
  ...
  private final int insertString() {
	...
	if (hash != (((window[strstart] << (2*HASH_SHIFT))
	  ...
	  System.out.println("hash inconsistent: "+hash+"/"
				  +window[strstart]+","
				  +window[strstart+1]+","
  ...
  private boolean findLongestMatch(int curMatch) {
	...
	&& strstart > 2*WSIZE - MIN_LOOKAHEAD)
      System.out.println("need lookahead");
    
    do {
      if (DeflaterConstants.DEBUGGING && curMatch >= strstart)
	System.out.println("future match");
	...
	if (lookahead == 0)
	  {
	    ...
	    if (DeflaterConstants.DEBUGGING && !flush)
	      System.out.println("Not flushing, but no lookahead");
	    huffman.flushBlock(window, blockStart, strstart - blockStart,
			       finish);
	    blockStart = strstart;
	    return false;
  ...
  public boolean deflate(boolean flush, boolean finish) 
  {
	...
	switch (comprFunc)
	  ...
	  case DEFLATE_SLOW:
	    ...
	    break;
	  default:
	    System.out.println("Error compr func");
	    progress = false;
	  }
      }
